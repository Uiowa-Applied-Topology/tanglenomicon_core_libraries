 ...
 */
typedef struct note_path_decode_char_dic_t {
    char *                char_class;
    char_handler_funptr_t funptr;
} note_path_decode_char_dic_t;
...
STATIC_INLINE_UINT8 note_path_decode_push_node();
STATIC_INLINE note_path_V4_label_e note_wptt_decode_get_v4_label(char label);
STATIC_INLINE bool note_path_decode_check_charset(const char *valid_chars,
                                                  const char str);
STATIC_INLINE_UINT8 note_path_decode_add_child();
STATIC_INLINE_UINT8 note_path_decode_space_handler(char **str);
STATIC_INLINE_UINT8 note_path_decode_opn_p_handler(char **str);
STATIC_INLINE_UINT8 note_path_decode_opn_a_handler(char **str);
STATIC_INLINE_UINT8 note_path_decode_opn_b_handler(char **str);
STATIC_INLINE_UINT8 note_path_decode_cls_handler(char **str);
STATIC_INLINE_UINT8 note_path_decode_weight_handler(char **str);

/************************** Encode path Function Declarations *****************/

STATIC_INLINE bool note_path_encode_stick_check(note_wptt_node_t *active_node_p);
STATIC_INLINE void note_path_encode_normalize_node_order(note_wptt_node_t *active_node_p);
STATIC_INLINE_UINT8 note_path_encode_insert_label(note_wptt_V4_label_e label,
                                                  char **str_p,
                                                  const char *buffer_end_p);

STATIC_INLINE_UINT8 note_path_encode_insert_char(char new_char,
                                                 char **str_p,
                                                 const char *buffer_end_p);

STATIC_INLINE_UINT8 note_path_encode_insert_int(int8_t new_int,
                                                char **str_p,
                                                const char *buffer_end_p);

STATIC_INLINE_UINT8 note_path_encode_insert_space(char **str_p,
                                                  const char *buffer_start_p,
                                                  const char *buffer_end_p);

STATIC_INLINE_UINT8 note_path_encode_insert_stick(note_wptt_node_t *active_node_p,
                                                  char **str_p,
                                                  const char *buffer_end_p);

STATIC_INLINE_UINT8 note_path_encode_complete_active_node(const note_wptt_node_t *active_node_p,
                                                          char **str_p,
                                                          size_t ordered_child_idx,
                                                          const char *buffer_start_p,
                                                          const char *buffer_end_p);
STATIC_INLINE_UINT8 note_path_encode_process_active_node(note_wptt_node_t *active_node_p,
                                                         char **str_p,
                                                         size_t ordered_child_idx,
                                                         const char *buffer_start_p,
                                                         const char *buffer_end_p);
...
/*!
 * @brief The path stack used in the encode and decode functions. This needs to be initialized at
 *the beginning of the using function.
 */
static note_path_node_t *wptt_node_stack[NOTE_WPTT_STACK_SIZE] = { NULL };

/*!
 * @brief The current number of elements on the path stack.
 */
static size_t path_stack_len = 0;

/*!
 * @brief Pointer to a buffer of path nodes. Used by decode path functions.
 */
static note_path_node_buffer_t *decode_buffer = NULL;
...
uint8_t note_path_decode(char *str, note_wptt_t *wptt)
{
    ...
    decode_buffer      = NULL;
    path_node_stack[0] = NULL;
    path_stack_len     = 0;

    /*Initialize the character checking dictionary*/
    const note_path_decode_char_dic_t charcheck_dict[] = {
        { "(",           &note_path_decode_opn_p_handler  },
        { "<",           &note_path_decode_opn_a_handler  },
        { "[",           &note_path_decode_opn_b_handler  },
        { ")>",          &note_path_decode_cls_handler    },
        { " ",           &note_path_decode_space_handler  },
        { "-0123456789", &note_path_decode_weight_handler }
    };
    ...
    else if ((NULL == path->node_buffer->buffer) || (0 == wptt->node_buffer->size))
    {
    ...
    else
    {
        ...
        decode_buffer = path->node_buffer;

        /*Initialize path root from buffer*/
        path->root        = &(decode_buffer->buffer[decode_buffer->idx]);
        path->root->order = NOTE_WPTT_ORDER_FORWARD;

        /*Check if the string has a V_4 label*/
        path->label = note_wptt_decode_get_v4_label(str[0]);
        if (NOTE_WPTT_V4_LABEL_NONE != path->label)
        {
            str++;
        }
        ...
        while (str[0] != '\0' && (NOTE_DEFS_DECODE_SUCCESS == retval))
        {
            ...
            {
                if (true == note_path_decode_check_charset(charcheck_dict[i].char_class, str[0]))
                {
...
uint8_t note_path_encode(note_wptt_t wptt, char *str, size_t buffer_size)
{
    ...
    else
    {
        ...
        buffer_start_p = str;
        retval        |= note_path_encode_insert_label(wptt.label, &str, buffer_end_p);
        /*While stack is not empty and string buffer has space*/
        while ((0 < path_stack_len) && (str < buffer_end_p) &&
               (NOTE_DEFS_ENCODE_SUCCESS == retval))
        {
            note_path_node_t *active_node_p = wptt_node_stack[wptt_stack_len - 1];
            ...
            {
                note_path_encode_normalize_node_order(active_node_p);
                size_t ordered_child_idx = child_idx_stack[path_stack_len - 1];
                /*Check if active node is complete*/
                if (active_node_p->number_of_children <
                    child_idx_stack[path_stack_len - 1])
                {
                    retval |= note_path_encode_complete_active_node(active_node_p,
                                                                    &str,
                                                                    ordered_child_idx,
                                                                    buffer_start_p,
                                                                    buffer_end_p);
                ...
                {
                    retval |= note_path_encode_process_active_node(active_node_p,
                                                                   &str,
                                                                   ordered_child_idx,
                                                                   buffer_start_p,
                                                                   buffer_end_p);
...
STATIC_INLINE note_path_V4_label_e note_wptt_decode_get_v4_label(char label)
{
    note_path_V4_label_e retval = NOTE_WPTT_V4_LABEL_UNINIT;
...
STATIC_INLINE_UINT8 note_path_decode_opn_p_handler(char **str)
{
    ...
    retval |= note_path_decode_push_node();
    retval |= note_path_decode_add_child();
    (*str)++;
    return retval;
}
...
STATIC_INLINE_UINT8 note_path_decode_opn_b_handler(char **str)
{
    ...
    {
        size_t  i;
        uint8_t pre_stick_stack_len = path_stack_len;
        (*str)++;
        for (i = 1; i < num_of_weights; i++)
        {
            ...
            path_node_stack[wptt_stack_len - 1]->weights[1u] =
        ...
        retval |= note_path_decode_push_node();
        retval |= note_path_decode_add_child();
        path_node_stack[wptt_stack_len - 1]->weights[0] = weights[0];
        path_stack_len = pre_stick_stack_len;
    }
    else
    {
        retval |= NOTE_STATUS_BLDR(
...
/*!
 * @brief Dictionary function to pop a node off of the path stack and advance to the next character
 *of the string.
 *
 * @param str A pointer to the pointer a the current character of the input string.
 * @return A status flag indicating successful completion of the subroutine.
...
/*!
 * @brief Initializes and pushes a node to the path stack.
 *
 * @return A status flag indicating successful completion of the subroutine.
 */
STATIC_INLINE_UINT8 note_path_decode_push_node()
{
    ...
    if ((decode_buffer->idx) + 1 < (decode_buffer->size) &&
        (path_stack_len + 1 < NOTE_WPTT_STACK_SIZE))
    {
        size_t            i;
        note_path_node_t *new_node = &decode_buffer->buffer[decode_buffer->idx];
        ...
        path_node_stack[wptt_stack_len] = new_node;
...
/*!
 * @brief Makes the active node on the path stack a child of the node below it on the stack.
 *
 * @return A status flag indicating successful completion of the subroutine.
 */
STATIC_INLINE_UINT8 note_path_decode_add_child()
{
    ...
    if ((0 < path_stack_len) && (wptt_stack_len < NOTE_WPTT_STACK_SIZE))
    {
        retval = NOTE_DEFS_DECODE_SUCCESS;
        ...
        if (1 < path_stack_len)
        {
            note_path_node_t *active_node_p = wptt_node_stack[wptt_stack_len - 2];

            /*Add child to active node*/
            active_node_p->children[active_node_p->number_of_children] =
                path_node_stack[wptt_stack_len - 1];
 ...
 *
 * @param active_node A path node and potential stick root.
 * @return The truthiness of whether the active node is the root of stick.
 */
STATIC_INLINE bool note_path_encode_stick_check(note_wptt_node_t *active_node_p)
{
    bool retval = false;
    note_path_node_t *child_node = active_node_p;
 ...
 */
STATIC_INLINE void note_path_encode_normalize_node_order(
    note_path_node_t *active_node_p)
{
    if (NOTE_WPTT_ORDER_REVERSE == active_node_p->order)
    {
        ...
        for (i = 0; 2 * i < active_node_p->number_of_children; i++)
        {
            ...
            note_path_node_t *child           = active_node_p->children[i];
                                                  ...
                                                  const char *buffer_end_p)
{
    ...
    retval = note_path_encode_insert_char('[', str_p, buffer_end_p);
    ...
    {
        retval |= note_path_encode_insert_int(weights[weight_idx - i], str_p, buffer_end_p);
        retval |= note_path_encode_insert_char(' ', str_p, buffer_end_p);
    }
    retval |= note_path_encode_insert_int(weights[0], str_p, buffer_end_p);
    retval |= note_path_encode_insert_char(']', str_p, buffer_end_p);
    return retval;
}
                                                  ...
                                                  const char *buffer_end_p)
{
    ...
    {
        retval |= note_path_encode_insert_char('i', str_p, buffer_end_p);
        break;
    }

    case NOTE_WPTT_V4_LABEL_X:
    {
        retval |= note_path_encode_insert_char('x', str_p, buffer_end_p);
        break;
    }

    case NOTE_WPTT_V4_LABEL_Y:
    {
        retval |= note_path_encode_insert_char('y', str_p, buffer_end_p);
        break;
    }

    case NOTE_WPTT_V4_LABEL_Z:
    {
        retval |= note_path_encode_insert_char('z', str_p, buffer_end_p);
        break;
    }
 ...
 * @return A status flag indicating successful completion of the subroutine.
 */
STATIC_INLINE_UINT8 note_path_encode_complete_active_node(
    const note_path_node_t *active_node_p,
    char **str_p,
    size_t ordered_child_idx,
    const char *buffer_start_p,
    const char *buffer_end_p)
{
    ...
    {
        retval |= note_path_encode_insert_space(str_p,
                                                buffer_start_p,
                                                buffer_end_p);
        retval |= note_path_encode_insert_int(active_node_p->weights[ordered_child_idx],
                                              str_p,
                                              buffer_end_p);
    }
 ...
 * @return A status flag indicating successful completion of the subroutine.
 */
STATIC_INLINE_UINT8 note_path_encode_process_active_node(
    note_path_node_t *active_node,
    char **str_p,
    size_t ordered_child_idx,
    const char *buffer_start_p,
    const char *buffer_end_p)
{
    uint8_t retval      = NOTE_DEFS_ENCODE_SUCCESS;
    bool    found_stick = note_path_encode_stick_check(active_node);

    /*Check if node has more children and start new node*/
    /* cppcheck-suppress negativeIndex */
    if (0 == child_idx_stack[path_stack_len - 1])
    {
        if (0 < active_node->number_of_rings)
        {
            retval |= note_path_encode_insert_char('<', str_p, buffer_end_p);
            retval |= note_path_encode_insert_int(active_node->number_of_rings, str_p,
                                                  buffer_end_p);
        }
        else if (true == found_stick)
        {
            retval |= note_path_encode_insert_stick(active_node, str_p, buffer_end_p);
            /*pop stack*/
            if (0 < path_stack_len)
            {
                path_stack_len--;
    ...
    if (false == found_stick)
    {
        ...
        if ((0 != active_node->weights[ordered_child_idx]) &&
            (false == found_stick))
        {
            retval |= note_path_encode_insert_space(str_p, buffer_start_p, buffer_end_p);
            retval |= note_path_encode_insert_int(active_node->weights[ordered_child_idx],
                                                  str_p,
                                                  buffer_end_p);
        }
        ...
        /*Push child to stack*/
        child_idx_stack[path_stack_len - 1]++;
        if (child_idx_stack[path_stack_len - 1] <= active_node->number_of_children)
        {
            path_stack_len++;
            path_node_stack[wptt_stack_len - 1] = active_node->children[ordered_child_idx];
            child_idx_stack[path_stack_len - 1] = 0;
        }
    }
    return retval;
}
