 ...
 */
typedef struct note_awptt_decode_char_dic_t
{
    char *char_class;
    char_handler_funptr_t funptr;

} note_awptt_decode_char_dic_t;
...
STATIC_INLINE_UINT8 note_awptt_decode_push_node();
STATIC_INLINE note_awptt_V4_label_e note_awptt_decode_get_v4_label(char label);
STATIC_INLINE bool note_awptt_decode_check_charset(const char *valid_chars,
                                                   const char str);
STATIC_INLINE_UINT8 note_awptt_decode_add_child();
STATIC_INLINE_UINT8 note_awptt_decode_space_handler(char **str);
STATIC_INLINE_UINT8 note_awptt_decode_opn_p_handler(char **str);
STATIC_INLINE_UINT8 note_awptt_decode_opn_a_handler(char **str);
STATIC_INLINE_UINT8 note_awptt_decode_opn_b_handler(char **str);
STATIC_INLINE_UINT8 note_awptt_decode_cls_handler(char **str);
STATIC_INLINE_UINT8 note_awptt_decode_weight_handler(char **str);

/************************** Encode path Function Declarations *****************/

STATIC_INLINE bool note_awptt_encode_stick_check(note_awptt_node_t *active_node_p);
STATIC_INLINE_UINT8 note_awptt_encode_get_child_ordered_idx(
    const note_awptt_node_t *active_node_p);
STATIC_INLINE_UINT8 note_awptt_encode_insert_label(note_awptt_V4_label_e label,
                                                   char **str_p,
                                                   const char *buffer_end_p);

STATIC_INLINE_UINT8 note_awptt_encode_insert_char(char new_char,
                                                  char **str_p,
                                                  const char *buffer_end_p);

STATIC_INLINE_UINT8 note_awptt_encode_insert_int(int8_t new_int,
                                                 char **str_p,
                                                 const char *buffer_end_p);

STATIC_INLINE_UINT8 note_awptt_encode_insert_space(char **str_p,
                                                   const char *buffer_start_p,
                                                   const char *buffer_end_p);

STATIC_INLINE_UINT8 note_awptt_encode_insert_stick(note_awptt_node_t *active_node_p,
                                                   char **str_p,
                                                   const char *buffer_end_p);

STATIC_INLINE_UINT8 note_awptt_encode_complete_active_node(
    const note_awptt_node_t *active_node_p,
    char **str_p,
    size_t ordered_child_idx,
    const char *buffer_start_p,
    const char *buffer_end_p);
STATIC_INLINE_UINT8 note_awptt_encode_process_active_node(
    note_awptt_node_t *active_node_p,
    char **str_p,
    size_t ordered_child_idx,
    const char *buffer_start_p,
    const char *buffer_end_p);
...
/******************************************************************************/
/*!
 * @brief The awptt stack used in the encode and decode functions. This needs
 *        to be initialized at the beginning of the using function.
 */
static note_awptt_node_t *awptt_node_stack[NOTE_AWPTT_STACK_SIZE] = {NULL};

/*!
 * @brief The current number of elements on the awptt stack.
 */
static size_t awptt_stack_len = 0;

/*!
 * @brief Pointer to a buffer of awptt nodes. Used by decode path functions.
 */
static note_awptt_node_buffer_t *decode_buffer = NULL;
...
uint8_t note_awptt_decode(char *str, note_awptt_t *awptt)
{
    ...
    decode_buffer = NULL;
    awptt_node_stack[0] = NULL;
    awptt_stack_len = 0;

    /*Initialize the character checking dictionary*/
    const note_awptt_decode_char_dic_t charcheck_dict[] = {
        {"(", &note_awptt_decode_opn_p_handler},
        {"<", &note_awptt_decode_opn_a_handler},
        {"[", &note_awptt_decode_opn_b_handler},
        {")>", &note_awptt_decode_cls_handler},
        {" ", &note_awptt_decode_space_handler},
        {"-0123456789", &note_awptt_decode_weight_handler}};

    /*Do basic error checking.*/
    if (strlen(str) <= 2)
    {
        retval = NOTE_STATUS_BLDR(
            NOTE_DEFS_DECODE_FAIL, NOTE_AWPTT_DECODE_EMPTY_STR);
    }
    else if (NULL == awptt)
    {
        retval = NOTE_STATUS_BLDR(
            NOTE_DEFS_DECODE_FAIL, NOTE_AWPTT_DECODE_NULL_DEST);
    }
    else if ((NULL == awptt->node_buffer.buffer) ||
             (0 == awptt->node_buffer.size))
    {
        retval = NOTE_STATUS_BLDR(
            NOTE_DEFS_DECODE_FAIL, NOTE_AWPTT_DECODE_BUFFER_ERROR);
    }
    else
    {
        ...
        decode_buffer = &(awptt->node_buffer);

        /*Initialize awptt root from buffer*/
        awptt->root = &(decode_buffer->buffer[0]);
        awptt->root->order = NOTE_AWPTT_ORDER_FORWARD;

        /*Check if the string has a V_4 label*/
        awptt->label = note_awptt_decode_get_v4_label(str[0]);
        if (NOTE_AWPTT_V4_LABEL_NONE != awptt->label)
        {
            str++;
        }
        ...
        while (str[0] != '\0' && (NOTE_DEFS_DECODE_SUCCESS == retval))
        {
            ...
            for (i = 0u; i < NOTE_AWPTT_DECODE_DICT_SIZE; i++)
            {
                if (true == note_awptt_decode_check_charset(
            ...
            if (i == NOTE_AWPTT_DECODE_DICT_SIZE)
            {
                retval |= NOTE_STATUS_BLDR(
                    NOTE_DEFS_DECODE_FAIL, NOTE_AWPTT_DECODE_BAD_STR);
            }
        }
    }
    return retval;
...
uint8_t note_awptt_encode(note_awptt_t awptt, char *str, size_t buffer_size)
{
    ...
    const char *buffer_end_p;
    const char *buffer_start_p;
    awptt_node_stack[0] = awptt.root;
    awptt_stack_len = 0x1u;
    ...
    {
        retval = NOTE_STATUS_BLDR(
            NOTE_DEFS_ENCODE_FAIL, NOTE_AWPTT_ENCODE_STR_BUF);
    }
    else if (NULL == str)
    {
        retval = NOTE_STATUS_BLDR(
    ...
    else
    {
        ...
        buffer_start_p = str;
        retval |= note_awptt_encode_insert_label(
            awptt.label, &str, buffer_end_p);
        /*While stack is not empty and string buffer has space*/
        while ((0 < awptt_stack_len) && (str < buffer_end_p) &&
               (NOTE_DEFS_ENCODE_SUCCESS == retval))
        {

            note_awptt_node_t *active_node_p =
                awptt_node_stack[awptt_stack_len - 1];

            /*Do basic error checking.*/
            if (NULL == active_node_p)
            {
                retval |= NOTE_STATUS_BLDR(
                    NOTE_DEFS_ENCODE_FAIL, NOTE_AWPTT_ENCODE_MALFORMED);
                break;
            }
            else if (active_node_p->order == NOTE_AWPTT_ORDER_UNINIT)
            {
                retval |= NOTE_STATUS_BLDR(
                    NOTE_DEFS_ENCODE_FAIL, NOTE_AWPTT_ENCODE_MALFORMED);
                break;
            ...
            {
                size_t ordered_child_idx =
                    note_awptt_encode_get_child_ordered_idx(active_node_p);
                /*Check if active node is complete*/
                if (active_node_p->number_of_children <
                    child_idx_stack[awptt_stack_len - 1])
                {
                    retval |= note_awptt_encode_complete_active_node(
                        active_node_p,
                        &str,
                        ordered_child_idx,
                        buffer_start_p,
                    ...
                    else
                    {
                        retval |= NOTE_STATUS_BLDR(
                            ...
                            NOTE_AWPTT_ENCODE_OVRUNDR_ERROR);
                    }
                }
                else
                {
                    retval |= note_awptt_encode_process_active_node(
                        active_node_p,
                        &str,
                        ordered_child_idx,
                        buffer_start_p,
        ...
        {
            retval |= NOTE_STATUS_BLDR(
                NOTE_DEFS_ENCODE_FAIL, NOTE_AWPTT_ENCODE_OVRUNDR_ERROR);
        }
    }
    return retval;
}
...
STATIC_INLINE note_awptt_V4_label_e note_awptt_decode_get_v4_label(char label)
{
    note_awptt_V4_label_e retval = NOTE_AWPTT_V4_LABEL_UNINIT;
    /*Check if the string has a V_4 label*/
    switch (label)
    {
    case NOTE_AWPTT_V4_LABEL_I_STR:
    {
        retval = NOTE_AWPTT_V4_LABEL_I;
        break;
    }
    case NOTE_AWPTT_V4_LABEL_X_STR:
    {
        retval = NOTE_AWPTT_V4_LABEL_X;
        break;
    }
    case NOTE_AWPTT_V4_LABEL_Y_STR:
    {
        retval = NOTE_AWPTT_V4_LABEL_Y;
        break;
    }
    case NOTE_AWPTT_V4_LABEL_Z_STR:
    {
        retval = NOTE_AWPTT_V4_LABEL_Z;
        break;
    }
    default:
    {
        retval = NOTE_AWPTT_V4_LABEL_NONE;
    }
    }
    return retval;
}
...
{
    uint8_t retval = NOTE_DEFS_DECODE_SUCCESS;
    retval |= note_awptt_decode_push_node();
    retval |= note_awptt_decode_add_child();
    (*str)++;
    return retval;
}
...
STATIC_INLINE_UINT8 note_awptt_decode_opn_b_handler(char **str)
{
           ...
           (NOTE_DEFS_DECODE_SUCCESS == retval))
    {
        ...
        {
            weights[num_of_weights] = strtol(*str, str, NOTE_AWPTT_INT_BASE);
            num_of_weights++;
            if (' ' == (*str)[0])
            {
                (*str)++;
    ...
    {
        size_t i;
        uint8_t pre_stick_stack_len = awptt_stack_len;
        (*str)++;
        for (i = 1; i < num_of_weights; i++)
        {
            retval |= note_awptt_decode_push_node();
            retval |= note_awptt_decode_add_child();
            awptt_node_stack[awptt_stack_len - 1]->weights[1u] =
                weights[num_of_weights - i];
        }
        retval |= note_awptt_decode_push_node();
        retval |= note_awptt_decode_add_child();
        awptt_node_stack[awptt_stack_len - 1]->weights[0] = weights[0];
        awptt_stack_len = pre_stick_stack_len;
    }
    else
    {
        retval |= NOTE_STATUS_BLDR(
...
STATIC_INLINE_UINT8 note_awptt_decode_opn_a_handler(char **str)
{
    ...
    if (NOTE_DEFS_ENCODE_SUCCESS == retval)
    {
        ...
        else if (0 == awptt_stack_len)
        {
            retval |= NOTE_STATUS_BLDR(
                NOTE_DEFS_DECODE_FAIL, NOTE_AWPTT_DECODE_OVRUNDR_ERROR);
        }
        else
        {
            note_awptt_node_t *active_node_p =
...
/*!
 * @brief Dictionary function to pop a node off of the awptt stack and advance
...
{
    uint8_t retval = NOTE_DEFS_DECODE_SUCCESS;
    note_awptt_node_t *active_node_p = awptt_node_stack[awptt_stack_len - 1];
    ...
    if (' ' == (*str)[0])
    {
        retval |= NOTE_STATUS_BLDR(
            ...
            NOTE_AWPTT_DECODE_OVRUNDR_ERROR | NOTE_AWPTT_DECODE_BAD_STR);
    }
    return retval;
}

/*!
 * @brief Initializes and pushes a node to the awptt stack.
 *
 * @return A status flag indicating successful completion of the subroutine.
 */
STATIC_INLINE_UINT8 note_awptt_decode_push_node()
{
    ...
    if ((decode_buffer->idx) + 1 < (decode_buffer->size) &&
        (awptt_stack_len + 1 < NOTE_AWPTT_STACK_SIZE))
    {
        size_t i;
        note_awptt_node_t *new_node = &decode_buffer->buffer[decode_buffer->idx];
        retval = NOTE_DEFS_DECODE_SUCCESS;
        /*Clear out new node*/
        for (i = 0; i < NOTE_AWPTT_DECODE_MAX_WEIGHTS; i++)
        {
            new_node->weights[i] = 0;
        }

        for (i = 0; i < NOTE_AWPTT_DECODE_MAX_CHILDREN; i++)
        {
            new_node->children[i] = NULL;
        }
        new_node->number_of_children = 0;
        new_node->number_of_rings = 0;
        new_node->order = NOTE_AWPTT_ORDER_FORWARD;

        /*Push new node to stack*/
        awptt_node_stack[awptt_stack_len] = new_node;
...
/*!
 * @brief Makes the active node on the awptt stack a child of the node below it
 * on the stack.
 *
 * @return A status flag indicating successful completion of the subroutine.
 */
STATIC_INLINE_UINT8 note_awptt_decode_add_child()
{
    ...
    if ((0 < awptt_stack_len) && (awptt_stack_len < NOTE_AWPTT_STACK_SIZE))
    {
        retval = NOTE_DEFS_DECODE_SUCCESS;

        /*Check if the active node is the root*/
        if (1 < awptt_stack_len)
        {
            note_awptt_node_t *active_node_p =
                awptt_node_stack[awptt_stack_len - 2];
 ...
 *
 * @param active_node A awptt node and potential stick root.
 * @return The truthiness of whether the active node is the root of stick.
 */
STATIC_INLINE bool note_awptt_encode_stick_check(note_awptt_node_t *active_node_p)
{
    bool retval = false;
    note_awptt_node_t *child_node = active_node_p;
    ...
    const note_awptt_node_t *active_node_p)
{
    ...
    if (NOTE_AWPTT_ORDER_FORWARD == active_node_p->order)
    {
        ordered_child_idx = child_idx_stack[awptt_stack_len - 1];
    }
    else if (NOTE_AWPTT_ORDER_REVERSE == active_node_p->order)
    {
                                                   ...
                                                   const char *buffer_end_p)
{
    ...
    note_awptt_node_t *child_node = active_node_p;
    size_t i = 0;
    retval = note_awptt_encode_insert_char('[', str_p, buffer_end_p);
    ...
    {
        retval |= note_awptt_encode_insert_int(
            weights[weight_idx - i], str_p, buffer_end_p);
        retval |= note_awptt_encode_insert_char(' ', str_p, buffer_end_p);
    }
    retval |= note_awptt_encode_insert_int(weights[0], str_p, buffer_end_p);
    retval |= note_awptt_encode_insert_char(']', str_p, buffer_end_p);
    return retval;
}
                                                   ...
                                                   const char *buffer_end_p)
{
    ...
    switch (label)
    {
    case NOTE_AWPTT_V4_LABEL_NONE:
    {
        break;
    }
    case NOTE_AWPTT_V4_LABEL_I:
    {
        retval |= note_awptt_encode_insert_char('i', str_p, buffer_end_p);
        break;
    }
    case NOTE_AWPTT_V4_LABEL_X:
    {
        retval |= note_awptt_encode_insert_char('x', str_p, buffer_end_p);
        break;
    }
    case NOTE_AWPTT_V4_LABEL_Y:
    {
        retval |= note_awptt_encode_insert_char('y', str_p, buffer_end_p);
        break;
    }
    case NOTE_AWPTT_V4_LABEL_Z:
    {
        retval |= note_awptt_encode_insert_char('z', str_p, buffer_end_p);
        break;
    }
    default:
    {
    ...
    const char *buffer_end_p)
{
    ...
    {
        retval |= note_awptt_encode_insert_space(
...
{
    uint8_t retval = NOTE_DEFS_ENCODE_SUCCESS;
    bool found_stick = note_awptt_encode_stick_check(active_node);

    /*Check if node has more children and start new node*/
    /* cppcheck-suppress negativeIndex */
    if (0 == child_idx_stack[awptt_stack_len - 1])
    {
        ...
        {
            retval |= note_awptt_encode_insert_stick(
    ...
    if (false == found_stick)
    {
        ...
        /*Push child to stack*/
        child_idx_stack[awptt_stack_len - 1]++;
        if (child_idx_stack[awptt_stack_len - 1] <=
            active_node->number_of_children)
        {
            awptt_stack_len++;
            awptt_node_stack[awptt_stack_len - 1] =
                active_node->children[ordered_child_idx];
            child_idx_stack[awptt_stack_len - 1] = 0;
        }
    }
    return retval;
}