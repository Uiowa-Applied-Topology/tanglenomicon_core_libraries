...
static void test_results_negative_result(void)
{
    comp_sum_parity_config_t test = compute_valid_set[0].cfg;
    uint8_t ret_val = comp_sum_parity_config(&test);

    TEST_ASSERT_EQUAL_UINT8_MESSAGE(ret_val, COMP_DEFS_CONFIG_SUCCESS, "Error in config.");
    const comp_sum_parity_result_t *rest = comp_sum_parity_result();
