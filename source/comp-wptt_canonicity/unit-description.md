...
abstract: A unit description for computation of weighted planar tangle tree canoncitiy.
---
...
stateDiagram-v2
    ...
    state "Check root canoncitiy" as crc
    state is_canon1 <<choice>>

    state walk{
        state "Compute vertex canoncitiy" as cvc
...
- A valid configuration is set, the following trees are configured:
    - One tree for each canoncitiy condition.
